...
namespace UzunTec.WinUI.Controls
{
    public class SimpleThemeBaseForm : Form
    {
        ...
        public Image LogoImage { get => this._logoImage; set { this._logoImage = value; this.UpdateRects(); this.Invalidate(); } }
        ...
        public int HeaderHeight { get => this._headerHeight; set { this._headerHeight = value; this.UpdateRects(); this.Invalidate(); } }
        ...
        public SimpleThemeBaseForm()
        {
            ...
            this._textFont = this.ThemeScheme.FormTitleFont;
            this._headerTextColor = this.ThemeScheme.FormTitleTextColor;
            this._headerColorDark = this.ThemeScheme.FormHeaderColorDark;
            this._headerColorLight = this.ThemeScheme.FormHeaderColorLight;
            this._borderColorDark = this.ThemeScheme.FormTitlePanelBackgroundColorDark.Darken(30);
            this._headerColorLight = this.ThemeScheme.FormTitlePanelBackgroundColorDark;

            this._borderWidth = BORDER_WIDTH;
        ...
        private void UpdateRects()
        {
            ...
            {
                this.headerRect = new RectangleF(0, 0, this.ClientRectangle.Width, this._headerHeight);
                ...
                {
                    iconData.rect = new RectangleF(this.headerRect.Right - BUTTON_WIDTH - buttonOffset, this.headerRect.Top, BUTTON_WIDTH, BUTTON_WIDTH + BUTTON_MARGIN * 2);
                ...
                this.textHeaderRect = headerRect.ApplyPadding(LEFT_TEXT_PADDING, 0, rightOffet, 0).ShrinkToSize(headerTextSize, this._headerTextAlign);

                if (this._logoImage != null)
                {
                    this.logoImageRect = this.headerRect.ApplyPadding(10, 5).ShrinkToSize(this._logoImage.Size, this._logoImageAlign);
                }
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            SolidBrush borderBrushLight = new SolidBrush(this._borderColorLight);
            SolidBrush borderBrushDark = new SolidBrush(this._borderColorDark);
            Pen borderPenLight = new Pen(borderBrushLight, this._borderWidth / 2);
            Pen borderPenDark = new Pen(borderBrushDark, this._borderWidth / 2);

            g.DrawRectangle(borderPenLight, this.borderRect.ToRect());
            g.DrawRectangle(borderPenDark, this.borderRect.ApplyPadding(-this._borderWidth / 2).ToRect());

            LinearGradientBrush headerBrush = new LinearGradientBrush(this.headerRect, this._headerColorDark, this._headerColorLight, LinearGradientMode.Vertical);
            g.FillRectangle(headerBrush, headerRect);

            if (this.hasHeaderText)
            {
                SolidBrush headerTextBrush = new SolidBrush(this._headerTextColor);
                g.DrawText(this.Text, this._textFont, headerTextBrush, this.textHeaderRect);
            }
            if (this._logoImage != null)
            {
                g.DrawImage(this._logoImage, this.logoImageRect);
            }
